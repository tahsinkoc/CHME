...
const DEFAULT_SNAPSHOT_DIR = path.resolve(process.cwd(), 'benchmark/snapshots_factual_mistral')
const DEFAULT_OUTPUT_PATH = path.resolve(process.cwd(), 'benchmark/results/factual_mistral_latest.json')
const DEFAULT_OPENAI_BASE_URL = 'https://api.mistral.ai/v1'
...
async function main(): Promise<void> {
  ...
  const model = parseStringArg('--model') ?? process.env.BENCHMARK_MODEL ?? DEFAULT_MODEL
  const openAIBaseUrl = resolveOpenAIBaseUrl(
    parseStringArg('--base-url'),
    process.env.MISTRAL_BASE_URL,
    process.env.OPENAI_BASE_URL
  )
  const openAIApiKey = resolveApiKey(process.env.MISTRAL_API_KEY, process.env.OPENAI_API_KEY)

  if (liveLlmGrading && !openAIApiKey) {
    throw new Error('Missing API key for live grading: set MISTRAL_API_KEY or OPENAI_API_KEY')
  }
...
function resolveOpenAIBaseUrl(cliBaseUrl: string | undefined, mistralBaseUrl: string | undefined, openAIBaseUrl: string | undefined): string {
  const raw = cliBaseUrl ?? mistralBaseUrl ?? openAIBaseUrl ?? DEFAULT_OPENAI_BASE_URL
  return raw.replace(/\/+$/, '')
}

function resolveApiKey(mistralApiKey: string | undefined, openAIApiKey: string | undefined): string {
  return (mistralApiKey ?? openAIApiKey ?? '').trim()
}
