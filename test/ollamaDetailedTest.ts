...
async function main(): Promise<void> {
  ...
  try {
    ...
    const ingestReport = await seedEngine.ingestAuto(testDir, { defaultCollectionId: 'general' })
    ...
    assert.ok(routingReport.lastIngestAssignments.length >= ingestReport.files)
    ...
    console.log(`Snapshots: ${saveReport.collections} collections -> ${snapshotDir}`)
    console.log(`Loaded collections: ${loadReport.collectionsLoaded} | stale reingested: ${loadReport.staleCollectionsReingested.join(', ') || 'none'}`)
